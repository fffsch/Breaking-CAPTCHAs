...
def apply_cv_preprocessing(image_tensor):
    """
    Applies OpenCV preprocessing to a single image tensor.
    Expected input is a [H, W, 1] float32 tensor (range 0.0-1.0).
    Returns a processed [H, W, 1] float32 array (range 0.0-1.0).
    """
    # 1. Convert tensor to NumPy and scale to 0-255 for OpenCV
    img_np = (image_tensor.numpy() * 255).astype(np.uint8)

    # 2. Apply your OpenCV operations
    # (Using the operations from your comments)
    
    # Median blur to reduce salt-and-pepper noise
    img_blurred = cv.medianBlur(img_np, 3) 
    
    # Adaptive thresholding is great for uneven lighting
    # This will create a black-and-white (binary) image
    ...
    # 3. Convert back to float32 (range 0.0-1.0) for TensorFlow
    img_processed_np = img_thresh.astype(np.float32) / 255.0
    
    # 4. Ensure it still has the channel dimension (OpenCV might remove it)
    if img_processed_np.ndim == 2:
        img_processed_np = np.expand_dims(img_processed_np, axis=-1)
        
    return img_processed_np

def encode_single_sample(img_path, label):
    # 1. Read and decode the image using pure TensorFlow
    img = tf.io.read_file(img_path)
    img = tf.io.decode_png(img, channels=1)
    img = tf.image.convert_image_dtype(img, tf.float32)

    # 2. Apply your custom OpenCV preprocessing
    img = tf.py_function(
        func=apply_cv_preprocessing,  # The Python function to run
        inp=[img],                     # The tensor(s) to pass to it
        Tout=tf.float32               # The output tensor type
    )
    
    # 3. Restore shape information (tf.py_function loses it)
    # This is crucial for the .batch() step later
    img.set_shape([None, None, 1])

    # 4. Continue with your TensorFlow-native operations
